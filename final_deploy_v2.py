...
def create_web_service_complete():
    print("Tam formatla web servisi oluşturuluyor...")
...
def create_with_minimal_config():
    print("\nMinimal konfigürasyonla deniyoruz...")
    
    secret_key = f"django-insecure-{uuid.uuid4().hex}"
    db_connection_string = f"postgres://{DB_USER}:postgres@{DB_ID}.internal:5432/{DB_NAME}"
    
    service_data = {
        "name": "cesa-hukuk-minimal",
        "ownerId": OWNER_ID,
        "type": "web_service",
        "region": "frankfurt",
...
if __name__ == "__main__":
    create_web_service_complete()
    create_with_minimal_config() 